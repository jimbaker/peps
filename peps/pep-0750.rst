...
.. note::
    ...
    interpolations into Python code, as was shown earlier. Instead it's as if
    one is writing ``interpolation_lambda: tag``, not ``lambda: tag``, where a
    hypothetical ``interpolation_lambda`` keyword variant uses annotation scope
    instead of the standard function scope.
...
.. note::
    ...
    aspect in how names in interpolation expressions are resolved. See
    `Interpolation Expression Evaluation`_.
